...
$regFileExts         = "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\FileExts\.sql"

removeRegistryKey $regFileExts

# $null = new-item -path $regFileExts
# $null = new-item -path $regFileExts\OpenWithProgids
# set-itemProperty -path $regFileExts\OpenWithProgids -name 'applications\Code.exe' -value ([byte[]] @()) -type none

$hkcu_sofware_classes = 'hkcu:\Software\Classes\.sql'
removeRegistryKey      $hkcu_sofware_classes
$null = new-item -path $hkcu_sofware_classes

set-itemProperty       $hkcu_sofware_classes -name '(default)' -value 'applications\Code.exe'
